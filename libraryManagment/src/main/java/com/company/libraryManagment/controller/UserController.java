...
import com.company.libraryManagment.utill.DateTracker;
import com.company.libraryManagment.utill.FineCalculator;
import com.company.libraryManagment.utill.ListInStringConverter;
...
public class UserController {
	...
	public String extendRequest(@RequestParam BigDecimal fineAmount, 
		...
		int extensionsLeft = maximumWeeksToExtend - book.getTimeExtended();
		long daysTooLate = dateTracker.daysTooLate(book.getReturnDate());
		
		if (book.getTimeExtended() < maximumWeeksToExtend && fineAmount.compareTo(BigDecimal.valueOf(0)) == 0 && book.getReservedByUser() == null) {
			book.setReturnDate(book.getReturnDate().plusDays(7 * weeksToExtend));
			book.setTimeExtended(book.getTimeExtended() + weeksToExtend);
	...
	public String saveBookReservations(@RequestParam String reservedBookIdsInString) {
		...
		dateTracker.setReservationDatesAndReservedByCurrentUserForMultipleBooks(reservedBookIds);
